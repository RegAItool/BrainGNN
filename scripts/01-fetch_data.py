...
# Note: subject_ID.txt will be created by the script
...
def apply_thresholding(connectivity_matrix, top_k_percent=0.1):
    # 获取下三角矩阵的值
    lower_tri = np.tril(connectivity_matrix, k=-1)
    # 计算阈值（保留top K%的连接）
    threshold = np.percentile(np.abs(lower_tri), (1 - top_k_percent) * 100)
    # 应用阈值
