...
"""
创建正确的6列DPV文件
BrainNet Viewer的DPV格式（实际上是.node格式）：x y z size color shape
"""

import numpy as np

def create_correct_6col_dpv():
    """创建正确的6列DPV文件"""
    ...
    # DPV格式：x y z size color shape（6列，去掉label）
    dpv_data = original_data[:, :6].copy()  # 只取前6列
    ...
    # 版本1: 标准6列DPV格式
    standard_file = 'standard_6col_activation.dpv'
    np.savetxt(standard_file, dpv_data, fmt='%.6f', delimiter='\t')
    print(f"✅ 标准6列DPV格式: {standard_file}")
    
    # 版本2: 大节点版本
    large_data = dpv_data.copy()
    large_data[:, 3] = 5.0  # 更大的节点
    large_file = 'large_6col_activation.dpv'
    ...
    contrast_file = 'contrast_6col_activation.dpv'
    ...
    top30_file = 'top30_6col_activation.dpv'
    ...
    # 6. 创建MATLAB测试脚本
    matlab_script = '''%% BrainNet Viewer - 6列DPV文件测试
% 测试正确的6列DPV文件
...
fprintf('=== BrainNet Viewer 6列DPV文件测试 ===\\n');
...
try
    % 测试标准版本
    data = load('standard_6col_activation.dpv');
    fprintf('✅ 标准6列DPV版本加载成功\\n');
    fprintf('   数据形状: %s\\n', mat2str(size(data)));
    fprintf('   列数: %d (正确)\\n', size(data, 2));
...
try
    if exist('BrainNet_View', 'file')
        ...
        fprintf('  Node: standard_6col_activation.dpv\\n');
    end
...
%% 5. 文件选择建议
fprintf('\\n=== 文件选择建议 ===\\n');
fprintf('• standard_6col_activation.dpv: 标准版本（推荐）\\n');
fprintf('• large_6col_activation.dpv: 大节点版本\\n');
fprintf('• contrast_6col_activation.dpv: 高对比度版本\\n');
fprintf('• top30_6col_activation.dpv: Top-30版本\\n');
...
fprintf('  3. 尝试large_6col_activation.dpv（更大节点）\\n');
fprintf('  4. 检查文件格式（必须是6列）\\n');

fprintf('\\n✅ 测试完成!\\n');
fprintf('现在应该能正确显示6列DPV激活图了!\\n');
'''
    
    with open('test_6col_dpv_files.m', 'w', encoding='utf-8') as f:
        f.write(matlab_script)
    
    print(f"MATLAB测试脚本已保存: test_6col_dpv_files.m")
    
    # 7. 创建使用说明
    instructions = '''# BrainNet Viewer - 6列DPV文件使用指南
...
'''
    ...
    with open('6col_DPV_File_Format_Guide.md', 'w', encoding='utf-8') as f:
        f.write(instructions)
    
    print(f"使用指南已保存: 6col_DPV_File_Format_Guide.md")
    
    print("\n✅ 6列DPV文件创建完成!")
    print("推荐使用: standard_6col_activation.dpv")
    print("如果仍有问题，请尝试: large_6col_activation.dpv")

if __name__ == "__main__":
    create_correct_6col_dpv() 